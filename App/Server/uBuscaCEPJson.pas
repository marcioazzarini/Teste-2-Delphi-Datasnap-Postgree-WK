...
type
  ...
  public
    ...
    class function BuscaCEP(const sCEP: string): TBuscaCEPJson;
  end;
...
{ TBuscaCEPJson }
{
class function TBuscaCEPJson.BuscaCEP(const sCEP: string): TBuscaCEPJson;
var
  s: string;
  lHTTP: TIdHTTP;
...
begin
  ...
  try
   ...
   Result := TJson.JsonToObject<TBuscaCEPJson>(s);
  finally
    lHTTP.Free;
  end;
end;
...
begin
  ...
  try
    ...
    if (IdHTTP.ResponseCode = 200) and (not (streamRet.DataString).Equals(sErro)) then
      Result := TJson.JsonToObject<TBuscaCEPJson>(
